...
file_path = os.path.join(script_dir, "..","resource", "ShortestLenth.txt")
leftlenth = []
...
class BaseFlowSimulator(Environment):
    def __init__(self, flow_net):
        ...
        self.will2node = [0] * flow_net.vcount()
    ...
    def add_agent(self, idx, start, dest, delay = 0, expected = 0):
        self.agents.append(Agent(idx, start, dest, expected))
        # self.will2node[start] += 1
...
class FlowSimulator(BaseFlowSimulator):
    ...
    def agent_proc(self, agent:Agent, delay=0, new_comer=True):
        ...
        while 1:
            ...
            self.will2node[next_node] += 1
            np.random.set_state(reserve_state)
            if next_node == -1:
                agent.reset((agent.start, agent.dest))
                break

            with self.graph.resource_btw(agent.current, next_node).request(priority = leftlenth[agent.current][agent.dest]) as req:
                yield req
                

                self.graph.resource_of(agent.current).get(1)
                ...
                self.will2node[next_node] -= 1      ##到达这个节点后，就从will to中删去
